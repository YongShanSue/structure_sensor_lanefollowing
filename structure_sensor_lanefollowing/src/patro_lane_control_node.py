...
class JoyMapper(object):
    ...
    def processSegments(self, segment_list_msg):
        self.segment_list = segment_list_msg
        self.publishControl()
        self.processButtons(joy_msg)
    ...
    def publishControl(self):
        ...
        car_cmd_msg.header.stamp = self.segment_list.header.stamp
        car_cmd_msg.v = 0.5
        self.yellow_check=0
        self.white_check=0
        self.whitedir=0
        ...
        x_vector=0
        y_vector=0   
        x_vector=segment_list_msg.segments[0].pixels_normalized[1].x - segment_list_msg.segments[0].pixels_normalized[0].x
        y_vector=segment_list_msg.segments[0].pixels_normalized[1].y - segment_list_msg.segments[0].pixels_normalized[0].y
        if x_vector!= 0 or y_vector!=0:
            self.yellow_check=1
            self.yellowdir=math.atan(y_vector/x_vector)
        x_vector=segment_list_msg.segments[1].pixels_normalized[1].x - segment_list_msg.segments[1].pixels_normalized[0].x
        y_vector=segment_list_msg.segments[1].pixels_normalized[1].y - segment_list_msg.segments[1].pixels_normalized[0].y
        if x_vector!= 0 or y_vector!=0:
            self.white_check=1
            self.whitedir=math.atan(y_vector/x_vector)
        segmentsum = yellow_check + white_check
        car_cmd_msg.omega = (self.yellowdir + self.white_check)/segmentsum*(8/-1/math.pi)
